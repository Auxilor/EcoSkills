...
class CommandFlySpeed(plugin: EcoPlugin) :
    PluginCommand(
        plugin,
        "flyspeed",
        "ecoskills.command.flyspeed",
        false
    ) {
    ...
    override fun onExecute(sender: CommandSender, args: List<String>) {
        ...
        if (args.isEmpty()) {
            sender.flySpeed = (1.0 / 10.0).toFloat()
            sender.sendMessage("§aTvá rychlost letu byla nastavena na §e100%")
            return
        }
        ...
        sender.flySpeed = (speed / 10.0).toFloat()
        sender.sendMessage("§aTvá rychlost letu byla nastavena na §e${(speed * 100).toInt()}%")
    }
    ...
    private fun computeMaxSpeed(player: Player): Float {
        ...
        val prefix = "ecoskills.command.flyspeed.max."
